...
async def monitor_resources(interval=5.0):
    ...
    while True:
        try:
            ...
            # Получение процента использования CPU
...
async def trace_memory_leaks(interval=10.0, top_n=5, snapshot_dir="memory_snapshots"):
    ...
    # Создание директории для снимков
    os.makedirs(snapshot_dir, exist_ok=True)
    
    while True:
        try:
            # Снимаем текущий снимок памяти
            snapshot2 = tracemalloc.take_snapshot()
            
            # Фильтрация для исключения системных библиотек
            snapshot2 = snapshot2.filter_traces((
                tracemalloc.Filter(True, "**/src/**"),  # Только ваш код
                tracemalloc.Filter(False, "**/lib/**"),  # Исключить стандартные библиотеки
                tracemalloc.Filter(False, "**/site-packages/**"),  # Исключить сторонние библиотеки
            ))
            
            if snapshot1 is not None:
                # Сравниваем снимки для выявления утечек
            ...
            # Логируем общую статистику памяти
            ...
            # Обновляем снимок для следующей итерации
            snapshot1 = snapshot2
