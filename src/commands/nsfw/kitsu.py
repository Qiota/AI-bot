...
async def translate_text(text: str, target_lang: str) -> str:
    """Переводит текст на указанный язык с помощью translatepy.
    ...
    try:
        ...
        logger.debug(f"Перевод успешен: {text[:50]}... -> {translated[:50]}... (сервис: {result.service})")
        return translated
...
class LanguageSelect(Select):
    ...
    async def callback(self, interaction: Interaction):
        ...
        logger.debug(f"Начало обработки LanguageSelect.callback для языка {self.values[0]}")
        ...
        try:
            ...
            logger.debug(f"Обработка LanguageSelect.callback завершена за {time.time() - start_time:.2f} сек")
        except Exception as e:
            ...
            logger.debug(f"Обработка LanguageSelect.callback завершена с ошибкой за {time.time() - start_time:.2f} сек")
...
async def kitsu(interaction: Interaction, bot_client, query: str, target_lang: str = "en") -> None:
    ...
    try:
        async with aiohttp_session() as session:
            ...
            if target_lang == "en":
                logger.debug("Используется оригинальный synopsis (английский)")
            anime = AnimeInfo(
